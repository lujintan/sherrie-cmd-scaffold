define(['jquery', 'extensions/lightapi', 'backbone', 'widget/spot'], function(jquery, lightapi, backbone,spot) {
	...
	var Gallery = backbone.View.extend({
		...
		initAtlas:function(){
			...
			this.spot = spot.getAtlasInstance({
		...
		changeTab: function(selection) {
			...
			lightapi.set("modules.spotlist.from", selection, function(err, res){
				if (err) {
					console.log(err);
				} else {
		...
		updateGallaries: function() {
			...
			lightapi.setValue("modules.spotlist", data, function(err, res){
				if (err) {
					console.log(err);
				} else {
					window.BuilderEvent && window.BuilderEvent.fire("modulepreview", "spotlist", true);
	...
	function init() {
		window.BuilderEvent.addEventListener("moduleselect", function(moduleName) {
			if (moduleName && moduleName == "spotlist") {
				gallery == null && lightapi.get("modules.spotlist", function(err, res) {
					if (err == null) {
						res = res || {};
						gallery = new Gallery({
							model: new GalleryModel(res),
							el: $("#mp-manage-spotlist")
						});
					}
				})
			}
